...
const url = `${globals.protocol}://${globals.host}/${globals.service}/${globals.destcprofileurl}`;
const urlForDel = `${globals.protocol}://${globals.host}/${globals.service}/${globals.delForDestinationConfig}`;
const urlForProxy = `${globals.protocol}://${globals.host}/${globals.service}/${globals.messagefilterurl}`
...
export const setSuccessMessage = (setsuccessCode) => {
    ...
    return {
        type: "SET_SUCCESS_DESTC_MESSAGE",
        payload: dataObject
    }
}
...
// http://127.0.0.1:8082/config/scp/v1/tracingDestinationDetailsConfigs/?operation=ADD&resume_marker_key=22.0&resume_marker_value=123
export function createDestinationConfig(dstcConfig){
    const ModifiedUrl=`${url}/?operation=ADD`
    return (dispatch) => {
...
export function updateDestcConfig(dstcConfig){
    const ModifiedUrl=`${url}/?operation=ADD`
    return (dispatch) => {
        dispatch(setSuccessFlag(false));
        axios({
