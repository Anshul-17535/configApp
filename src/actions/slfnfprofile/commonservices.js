...
export const dataFetched = (responseData) => {
    ...
    let finalResponse = [];
    if(!!responseData) {
      finalResponse=_.pluck(responseData, 'nfProfileCache')
    }
    return {
        type: "GET_ALL_RECORDS_SLF_NFPROFILE",
...
export const groupFetchErrored = (errorId) => {
    ...
    return (dispatch) => {
        axios({
            ...
            return Promise.all([
                ...
                dispatch({
                    type:"SET_ALL_SLF_NFPROFILE_DATA",
  ...
  export function createNfProfileConfig(NfProfileConfig){
    const ModifiedUrl=`${nfprofileurl}?operation=POST`
    return (dispatch) => {
        dispatch(setSuccessFlag(false));
        axios({
            method: 'post',
            url: ModifiedUrl,
            data:NfProfileConfig,
            headers: {'Content-type':'application/json',
                'Accept': 'application/json',
                'Authorization':  global.authToken,
                'RequestSource':"GUI",
  ...
  export function updateNfProfileConfig(NfProfileConfig){
    const ModifiedUrl=`${nfprofileurl}?operation=POST`
    return (dispatch) => {
        dispatch(setSuccessFlag(false));
        axios({
  ...
  export function deleteSlfNfProfileConfig(NfInstanceId){
    
    const ModifiedUrl=`${nfprofileurl}/${NfInstanceId}`
    return (dispatch) => {
        dispatch(setSuccessFlag(false));
        axios({
            method: 'delete',
