...
import {setDialogState} from "../../actions/rulesengine";
import {setErrorOnValidation} from '../../actions/messagefilter'
import {createTracingConfig,updateTracingConfig} from '../../actions/messagefilter/commonServices'
import {bindActionCreators} from "redux";
import MFNameState from './MFNameState'
import MFGroup from './MFGroup'
import {isTracingConfigNameEmpty,isTracingConfigTagEmpty,isFilterNameOrActionEmpty,isNumberOfFiltersZero,isContextOrNameEmptyInAction,isContextOrNameEmptyInCriteria,isValueMissingInAction,isValueMissingInCriteria,isMessageFilterNameNotUnique,isDuplicateActionsPresent} from './validationFunctions'
import "../../styles/messageFilter/AddEditDialog.css";
...
class AddEditDialog extends Component{
    ...
    handleRenderFooterSaveClick=()=>{
        ...
        switch(true){
            case isTracingConfigNameEmpty(this.props.tracingConfig):
                ...
                break;
            case isTracingConfigTagEmpty(this.props.tracingConfig):
                this.setState({ValidationErrorMessage:"Empty Config type"},()=>{
                    this.props.setErrorOnValidation(true)
                })
                break; 
    ...
    renderFooter=()=>{
        return <div>
            <ButtonCCFK text="SAVE" disabled={this.props.errorState.length>0 || this.props.dialogType==="view"} onClick={this.handleRenderFooterSaveClick} variant="call-to-action" />
        </div>
    }
    ...
    render(){
        return(
            <DialogCCFK
                title={`${this.props.dialogType==="edit"?"Edit":"Add"} Tracing Configuration`}
...
function mapStateToProps(state) {
    return{
        ...
        configsList:state.mfnewEditDialog.tracingConfigs,
        ...
        loading:state.mfnewEditDialog.loading
    }
}
