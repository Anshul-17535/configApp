...
export const isTracingConfigTagEmpty=(tracingConfig)=>{
    return tracingConfig.filterGroup.msgFilterType === "" || tracingConfig.filterGroup.msgFilterType === null || tracingConfig.filterGroup.msgFilterType === undefined
}
...
export const isValueMissingInAction=(filtersArray)=>{
    ...
    filtersArray.map((filterObj)=>{
        filterObj.filterActions.map(ActionObj=>{
            let myValue= "nrfMessageType" in ActionObj.parameterNamesAndValues;
            if(myValue){
                myValue=ActionObj.parameterNamesAndValues.profileName != ""
            }
            if( Object.values(ActionObj.parameterNamesAndValues).some(e=>e.trim().length===0)){
                if(!myValue){isValueMissingInAction=true}
            }
        })
    })
