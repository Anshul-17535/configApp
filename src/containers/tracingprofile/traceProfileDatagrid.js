...
import {setDeleteAlertState,setDialogType,setDialogState,setCurrentThrProfileDataOnEdit,setCurrentThrProfileDataOnView} from '../../actions/tracingprofile';
import {setErrorOnCall,deleteThrConfig} from '../../actions/tracingprofile/commonservices';
import "../../styles/messageFilter/ReDataGrid.css";
import rulesData from '../../actions/tracingprofile/tracingProfileGridData.json';
...
let srv;
class TraceProfileDataGrid extends Component {
    constructor(props) {
        super(props);
        srv = this;
        srv.state={
    ...
    EditFunc=(ConfigName)=>{
        ...
        this.props.setCurrentThrProfileDataOnEdit(ConfigName)
        this.props.setDialogState(true);
    }

    viewFunc=(ConfigName)=>{
        srv.props.setDialogType('view');
        this.props.setCurrentThrProfileDataOnView(ConfigName)
        this.props.setDialogState(true);
    }
    ...
    onConfirm = () => {
        ...
        sendData.tracingProfileConfigs.tracingProfileConfig[0].name=this.state.selectedThrConfigNameToDelete
        srv.props.deleteThrConfig(sendData.tracingProfileConfigs.tracingProfileConfig[0].name);
        srv.props.setDeleteAlertState(false);
    }
    componentDidMount(){
        srv.props.setDeleteAlertState(false);
    ...
    render(){
        ...
        return (
            <div style={{height: '595px'}}>
                ...
                />
                {srv.props.dataGrid.showDeleteAlert && srv.alertConfirmationPopup("Delete Tracing Profile", "Are you sure , you wish to delete the configuration ? ")}
...
function mapStateToProps(state){
    return {
        rowData: state.tcpdataGrid.rowData,
        dataGrid : state.tcpdataGrid,
        apiError: state.tcpdataGrid.apiError,
    }
}
...
function matchDispatchToProps(dispatch){
    return bindActionCreators({
        ...
        setDialogState,
        setErrorOnCall,
        setCurrentThrProfileDataOnEdit,
        setCurrentThrProfileDataOnView,
        deleteThrConfig,
    }, dispatch)
}

export default connect(mapStateToProps,matchDispatchToProps)(TraceProfileDataGrid);